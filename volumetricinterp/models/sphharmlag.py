...
# Model class should be initialized with a config file object that contains all parameters needed to initialize Model
# (max # basis, centers, ect.) other modules assume this module can handle multidimensional arrays, so if there needs
# to be any flattening/reforming, do it internally
...
class Model(object):
    ...
    Attributes:
        ...
        C: array of fit coefficients - must be nbasis long
        dC: covariance matrix of fit coefficients - must be nbasis X nbasis
    ...
    def __init__(self, config_file, maxk=None, maxl=None):
    ...
    def basis(self, gdlat, gdlon, gdalt):
        ...
        Parameters:
            gdalt: [ndarray(npoints)]
            gdlon: [ndarray(npoints)]
            gdlat: [ndarray(npoints)]
        Returns:
            A: [ndarray(npoints,nbasis)]
                array of basis functions evaluated at all input points
        Notes:
            - Something clever could probably be done to not recalculate the full expression when incrimenting n does
            not result in a change in k, or similar. All the evaluations of special functions here make it one of the
            slowest parts of the code.
        """

        # Transform coordinates from (lat lon, alt) to (z, theta, phi)
        z, theta, phi = self.transform_coord(gdlat.flatten(), gdlon.flatten(), gdalt.flatten())

        # Craete basis function matrix (A).
        A = []
    ...
    #     t_int = lambda t: 1/np.sin(t)**3*(-1*vi*(vi*np.cos(t)**2+vi+1)*sp.lpmv(mi,vi,np.cos(t))+vi*(vi+mi)*np.cos(t)*
    #     sp.lpmv(mi,vi-1,np.cos(t))+vi*(vi-mi+1)*np.cos(t)*sp.lpmv(mi,vi+1,np.cos(t)))*(-1*vj*(vj*np.cos(t)**2+vj+1)*
    #     sp.lpmv(mj,vj,np.cos(t))+vj*(vj+mj)*np.cos(t)*sp.lpmv(mj,vj-1,np.cos(t))+vj*(vj-mj+1)*np.cos(t)*
    #     sp.lpmv(mj,vj+1,np.cos(t)))
    ...
    def omega_ij(self, ni, nj):
        ...
        def T(v, m, t):
            return -m * (m - 1) * sp.lpmv(m, v, np.cos(t)) + 2 * (m - 1) * (v + m) * (v + m - 1) * np.cos(t) / np.sin(
                t) * sp.lpmv(m - 1, v, np.cos(t)) + (v + m) * (v + m - 1) ** 2 * (v + m - 2) * sp.lpmv(m - 2, v,
                                                                                                       np.cos(t))

        def Z(k, z):
            return 1 / (2 * RE * z) * (
                        (4 * RE * z * k - 2 * RE * z ** 2 - k + z / 2 - 2 * k / z + 2 * k ** 2 / z) * sp.eval_laguerre(
                    k, z) + (-4 * RE * z * k + 4 * k / z - 4 * k ** 2 / z + k) * sp.eval_laguerre(k - 1, z) + 2 * k * (
                                    k - 1) / z * sp.eval_laguerre(k - 2, z))
    ...
    def transform_coord(self, gdlat, gdlon, gdalt):
        ...
        Rp = np.array([x, y, z])
        Rr = np.array(
            [R * np.cos(theta0) + np.cross(k, R) * np.sin(theta0) + k * np.dot(k, R) * (1 - np.cos(theta0)) for R in
             Rp.T]).T
    ...
    def inverse_transform(self, R0, vec):
        ...
        vx, vy, vz = cc.vector_spherical_to_cartesian(vec.T[0], vec.T[1], vec.T[2], (R0[0] / 100. + 1.) * RE, R0[1],
                                                      R0[2])
        ...
        rr = np.array(
            [R * np.cos(theta0) + np.cross(k, R) * np.sin(theta0) + k * np.dot(k, R) * (1 - np.cos(theta0)) for R in
             Rc.T]).T
        vr = np.array(
            [v * np.cos(theta0) + np.cross(k, v) * np.sin(theta0) + k * np.dot(k, v) * (1 - np.cos(theta0)) for v in
             vc.T]).T
